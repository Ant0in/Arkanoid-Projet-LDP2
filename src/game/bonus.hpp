...
class BonusInterface {
   ...
   public:
    BonusInterface(const Position2D& pos, float size, int active_duration, float falling_speed,
                   bool is_active = false, bool is_spawned = false)
...
class DuplicationBonus : public BonusInterface {
   public:
    DuplicationBonus(const Position2D& pos = DEFAULT_CORNER_POS, float size = BONUS_SIZE,
                     int active_duration = BONUS_DEFAULT_DURATION,
                     float falling_speed = BONUS_FALLING_SPEED, bool is_active = false,
...
class PlayerBonus : public BonusInterface {
   public:
    PlayerBonus(const Position2D& pos = DEFAULT_CORNER_POS, float size = BONUS_SIZE,
                int active_duration = BONUS_DEFAULT_DURATION,
                float falling_speed = BONUS_FALLING_SPEED, bool is_active = false,
                bool is_spawned = false)
        : BonusInterface(pos, size, active_duration, falling_speed, is_active, is_spawned) {}
...
class ResizeBonus : public BonusInterface {
   public:
    ResizeBonus(const Position2D& pos = DEFAULT_CORNER_POS, float size = BONUS_SIZE,
                int active_duration = BONUS_DEFAULT_DURATION,
                float falling_speed = BONUS_FALLING_SPEED, bool is_active = false,
                bool is_spawned = false)
        : BonusInterface(pos, size, active_duration, falling_speed, is_active, is_spawned) {}
