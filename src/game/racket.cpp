...
const SolidRectangle& Racket::getHitbox() const {
    return _hitbox;
}

Position2D Racket::getPosition() const {
    return getHitbox().getPosition();
}
void Racket::setPosition(const Position2D& pos) {
...
float Racket::getWidth() const {
    return getHitbox().getWidth();
}

void Racket::setHeight(float h) {
    getHitbox().setHeight(h);
}
void Racket::setWidth(float w) {
    getHitbox().setWidth(w);
}

float Racket::getSensibility() const {
    return _sensibility;
}
void Racket::setSensitivity(float s) {
    _sensibility = s;
}

Position2D Racket::getCenterPosition() const {
...
Position2D Racket::calculateNewPosition(const Action& action) {
    ...
    // by default, we don't move the racket
    switch (action) {
        case Action::LEFT:
