...
SolidCircle& Ball::getHitbox() {return _hitbox;}
const SolidCircle& Ball::getHitbox() const {return _hitbox;}
...
std::tuple<float, float> Ball::getVelocity() const {return std::make_tuple(_x_velocity, _y_velocity);}
void Ball::setVelocity(float xv, float yv) {_x_velocity = xv; _y_velocity = yv;}

float Ball::getSpeed() const {return _speed;}
void Ball::setSpeed(float s) {_speed = s;}

bool Ball::isAlive() const {return _is_alive;}
void Ball::setAlive(bool flag) {_is_alive = flag;}
